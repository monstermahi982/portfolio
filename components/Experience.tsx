  ...
  return (
    <>
      <div className="relative w-full">
        {/* Vertical line */}
        <div className="absolute left-1/2 md:left-1/3 top-0 h-full w-1 bg-gradient-to-b from-transparent via-slate-300 to-transparent transform -translate-x-1/2" />
        ...
        {timeline.map((item: TimelineItem, index: number) => (
          ...
          >
            {/* Image & time block */}
            <div className="w-full md:w-1/3 md:pr-8 flex flex-col items-center md:items-end text-center md:text-right">
              <div className="w-32 h-16 rounded-2xl shadow-md flex items-center justify-center mb-4 grayscale hover:grayscale-0 hover:scale-125 transition duration-300">
                <Image
                  src={item.image}
                  alt={item.name}
                  width={"100"}
                    ...
                    {item.stack.map((tech, idx) => (
                      <div key={idx} className="group">
                        <Image
                          ...
                          height={20}
                          title={tech.name}
                          className="grayscale group-hover:grayscale-0 hover:scale-150 transition duration-300"
                        />
                      </div>
                    ))}
                  </div>
